...
(`r signif(coef(lm(d$rep_es~d$original_ES_d-1))[1], digits = 2)*100`\% of the original [`r signif(confint(lm(d$rep_es~d$original_ES_d-1))[1], digits = 2)*100` - `r signif(confint(lm(d$rep_es~d$original_ES_d-1))[2], digits = 2)*100`]). 
...
```{r, fig.height=9, fig.width=6, fig.cap="Side-by-side comparison of plots for each attempted replication. All error bars are standard error of the mean. Original data were estimated from figures when not otherwise available. Replications are sorted and color coded by subjective replication success."}
