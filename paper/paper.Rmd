...
Replicability is a core value for empirical research and there is increasing concern throughout psychology that more independent replication is necessary [@osc2015;@wagenmakers2012]. Yet under the current incentive structure for science, replication is not typically valued valued for publication [@makel2012], or in metrics of research productivity [@koole2012]. One potential solution to this problem is to make replication an explicit part of pedagogy: that is, to teach students about experimental methods by asking them to run replication studies [@frank2012;@grahe2012]. Despite enthusiasm for this idea [@lebel2015;@everett2015;@standing2016;@king2016], there is limited data beyond anecdotal reports or individual projects [@phillips2015;@lakens2013] to support its efficacy in producing wide-scale pedagogical adoption.

In the current article, we address the practical barriers of completing replcations as part of required coursework and discuss the methodological decisions that replication projects must address more braodly.  Towards this aim, report the results of replication projects conducted in a graduate-level experimental methods course. Students conducted replications of published articles from the 2015 volume of the journal *Psychological Science*. These studies provide insight into both the difficulties of pedagogical replications and their promise as a method for improving the robustness of psychological research. 
...
We also describe our process for conducting replications as part of classroom pedagogy. Although mentorship in experimental methods is an important part of the standard advising relationship, the classroom context allows for elucidation of general principles of good research and discussion of how they can be modified to fit specific instances. And replication research in particular illustrates a number of important concepts -- experimental design, power analysis, reporting standards, and preregistration, among others -- more directly than open-ended projects, which require new conceptual development [see @frank2012 for extended argument]. There are significant limitations on what can be done in a single term, within the constraints of a course budget and the instructors' expertise. Nevertheless, were this approach implemented more widely, we believe the dividends paid to the field as a whole---both scientific and educational---would be considerable.

To encourage others to share our materials, our course outline, project templates, and assignments are available publicly at [https://osf.io/98ta4/files/](https://osf.io/98ta4/files/). In addition, all of the most recent lecture slides and materials are avaialable on our course website at [http://psych254.stanford.edu](). We hope that others will share and reuse our materials as they consider how best to design a course customized to their context. 
...
Sofer et al. & 1 & Yes & Yes & No & 48 & Other &  95 & 4.33 & osf.io/5wmhn/ \\ 
...
Zaval, Markowitz, \& Weber & 1 & Yes & Yes & Yes & 312 & Original & 321 & 5.00 & osf.io/degwa/ \\ 
   \hline
\end{tabular}
\caption{\label{tab:summary} Summary characteristics of original studies and our replications, along with links to project pages for individual projects. All project materials available publicly at osf.io/98ta4/files/. * marks projects where the number of trials was modified.} 
\label{tab:summary}
\end{table}
...
RXDH, ENS, and MCF designed the project, supported data planning and data collection for all projects, analyzed the data, and wrote the paper. CA, JMA, RC, EH, MK, AL, SR, JR, SS, JS, and JT planned their individual projects, programmed their studies, collected their data, and analyzed their data.
...
Thanks to the Stanford Department of Psychology and the Vice Provost for Graduate Education for funding to support the class. We are grateful to the authors of the original studies who provided materials and gave comments on a draft of this manuscript.
...
We begin by describing the details of our course projects to give a sense of what a classroom replication project entails. We then discuss broader decision points for the course design that other instructors might consider. Lastly, we report the results our replications, highlighting the importance of wide-scale replication.
...
For `r sum(project_plan$author_contact != "")` studies, however, students contacted the authors to request either materials or clarification of methods using a template email that was customized by the students and reviewed by the instructors (this template is included in our course materials). 
Responses were received in all but one case, all within a matter of days.  

## Preregistration and Review

In addition to as-needed guidance on projects, students advanced through a formal review process in which each student's work was inspected and critiqued several times by both the instructor (MCF) and the teaching assistants (ENS and RXDH). 
After selecting their replication targets, students' first milestone was to fill in the methods section of the @osc2015 replication report template, which was reviewed for feasability and concreteness. 
...
Despite the general emphasis in meta-analytic and reproducibility efforts on aggregating a single effect size of interest [@osc2015;@lipsey2001], it was often challenging for us to identify a single key statistical test for each study. Usually a substantial number of statistical tests were conducted, often within one or several regression models with multiple specifications. Thus, we interpret "key statistical test" results with caution (see Empirical Results and Discussion below). Despite this interpretive difficulty, following previous work, we attempted to use a single statistical test as our target for replication planning and analysis.
...
Because they were conducted by students within the constraints of a course, our studies had a number of limitations, including at least: 1) limited time for iterative piloting and adjustment, 2) limits on funding for larger samples, and 3) limits on domain expertise with respect to the specific effects that were chosen. But within these limitations, our group was able to produce a set of relatively close replications with generally good statistical power. Thus, our work here shows what is possible for motivated students using freely-available tools and resources.

## Discussion: Pedagogical Implications

Above we have shown that classroom replication projects are possible. We now present information that we hope will reduce barriers to entry and encourage more researchers and instructor to include replications as part of coursework.  This includes practical recommendations for implementation in a variety of classroom, key decision that instructors must make, and common issues that may arise throughout the projects and ways to mitigate or prevent these issues. 

### Models for students of different levels

Though others' courses that incorporate replication projects will look quite different from our own, we recommend having the replication as a centerpiece of the course and giving ample opportunity for feedback throughout the study design. Without a central focus on the replication project, students would not have the time or motivation to complete a high quality project; without multiple opportunities for instructor feedback, we would have far less confidence in the quality of the final projects.

That said, we beleive there are many ways to adapt the project format based on the students and constraints of the class.  In Table \ref{tab:classmodels} below, we present mulitple ways in which replication projects may be included in classrooms.  Though we do not go into detail on all of the specifics of what each of these courses might entail, we hope this provides an overview of the possibilities inherent in including replication work as part of student learning.  We next turn to detailed discussion on a few we believe most pertinent to instructors.
...
Students & \multicolumn{1}{p{2in}}{\raggedright Undergraduate} & \multicolumn{1}{p{2in}}{\raggedright Advanced Undergraduate or Early Graduate} & \multicolumn{1}{p{2in}}{\raggedright Motivated Graduate} \\

Goal & \multicolumn{1}{p{2in}}{\raggedright Gain experience with research process} & \multicolumn{1}{p{2in}}{\raggedright Conduct collaborative research} & \multicolumn{1}{p{2in}}{\raggedright Learn best practices and tools} \\

Grouping & \multicolumn{1}{p{2in}}{\raggedright Full Class OR Medium groups (7-8 students)} & \multicolumn{1}{p{2in}}{\raggedright Small groups (2-4) or individual} & \multicolumn{1}{p{2in}}{\raggedright Individual} \\

Project Selection & \multicolumn{1}{p{2in}}{\raggedright One class replication OR pre-specified list} & \multicolumn{1}{p{2in}}{\raggedright Curated list to choose from} & \multicolumn{1}{p{2in}}{\raggedright Open to student choice*} \\

Data source & \multicolumn{1}{p{2in}}{\raggedright University or classroom participant pool} & \multicolumn{1}{p{2in}}{\raggedright In-person or online convenience sample} & \multicolumn{1}{p{2in}}{\raggedright Targetted sampling of original populations} \\

Workflow tools & \multicolumn{1}{p{2in}}{\raggedright Use familiar or GUI tools OR instructors implement studies} & \multicolumn{1}{p{2in}}{\raggedright Teach key tool plus introduction of others} & \multicolumn{1}{p{2in}}{\raggedright Teach full set of free open-source software} \\

Review & \multicolumn{1}{p{2in}}{\raggedright Standardized format for submission} & \multicolumn{1}{p{2in}}{\raggedright Group and minimal instructor review} & \multicolumn{1}{p{2in}}{\raggedright Multiple reviews between students and with instructors} \\

Pre-registration & \multicolumn{1}{p{2in}}{\raggedright Pre-registration by instructor} & \multicolumn{1}{p{2in}}{\raggedright Pre-registration by instructor (drafted by students)} & \multicolumn{1}{p{2in}}{\raggedright Pre-registration by students (with instructor review)} \\

Dissemination & \multicolumn{1}{p{2in}}{\raggedright If up to quality standard} & \multicolumn{1}{p{2in}}{\raggedright Upon review of instructors} & \multicolumn{1}{p{2in}}{\raggedright Discuss dissemination as a class} \\

\hline

\end{tabular}
\caption{\label{tab:classmodels} *This can also be done with random selection with sampling criteria} 
...
In our class, students selected projects based on interest, rather than via a systematic sampling strategy. After selecting a paper, students chose a particular study from within that paper based on their judgment of interest and feasibility. We encouraged students with little programming background to opt for methodologically simple studies, and more advanced students to take on a challenging design. However, this sample also reflects what is of most interest to beginning researchers, and we suspect may be a better sampling frame to better understand particularly impactful effects to be built upon in future research. The modal study chosen was the first, but several students chose later studies in a set as well.  Thus, all interpretation of our findings should be with respect to this non-probability sample.  This sampling strategy may thus bias our results towards more exciting or novel findings, as well as those that were more feasible for students.

The set of studies could be curated more or less for students based on expertise and constraints.  At the other extreme from our small graduate class, in a large undergraduate class it may make logistical sense for the instructors to choose 1-3 replication projects that students can select from. This would allow the instructor to be familiar with all replication projects taking place, and provide more structured discussion and problem-solving based on the challenges groups may run into.  

### Author contact

Over half of our students reached out to original authors to request study materials or clafication on methods and/or analysis.  For the purposes of this class, we chose not to require contact with all original authors. Methods and materials should be open to allow for replication and incremental science [@nosek2015], and hoped to make the pedagogical point that good research should not require a researcher to return your email, and make students feel empowered that they can engage with and build on the published literature directly.  That is, we chose to do good-faith replications given all information in the original articles and supplemental online materials.  Only in cases where there are issues about access to materials or ambiguities in design/analysis were authors contacted.  

However, we acknowledge there are benefits of contacting all authors.  There are two main methods of doing so. First is as a professional courtesy by letting the authors know that their study is being replicated.  This not only provides goodwill (as replication are perceived by some to be adversarial; see Kahneman commentary?), it would also allow the original authors to pre-register their hypotheses or concerns with the replication study as experts in the underlying theory. Second is as a request for methodological feedback prior to replication. This is a trickier issue on many fronts.  By requesting feedback, this perpetuates insider information or lab-specific folklore on methodology that produces the greatest effects. When these assumptions or limitations of a theory or paradigm are not made explicit in publically-available format, this does not count as a record of science. In fact, some replications have prompted these assumptions to be made explicit by the original authors (CITE?).  In addition, when original materials of a study are not provided, it is on the reader to assume all neccessary information is provided to replicate, and is not due to idiosyncracies. Thus, we would generally advice against contacting authors for feedback or materials expect when replication is not viable without them.

### Subject population

We chose to use Amazon Mechanical Turk (AMT) as a platform for our replication studies. This choice was made to facilitate the process of recruiting samples large enough to enable replications of between-subjects designs (which typically require large samples) as well as to help students learn about a valuable resource for recruitment whose use requires some specialized knowledge. Our general class structure does not depend on the use of AMT, however, and the use of this particular online sample should be viewed as limiting the interpretation of some results. Other classes many find participant pools or recruitment of community samples more feasible and appropriate.

Repeat administration of empirical work on AMT has been raised as an issue in some prior work [e.g., @chandler2014], but tracking participation in specific paradigms is an open challenge. We did not ask participants whether they had participated in similar research previously, as we suspected that asking this sort of question would lead to a large number of inaccurate responses. In addition, half of the AMT population is estimated to change every six months [@stewart2015], and we expected that most of the studies in our sample that had been conducted on AMT had been performed at least a year previously (though see Supplemental Material for discussion of one particular paradigm that has been used more recently).

As much of psychological research is now conducted online, and we know of few PhD programs that require explicit training in online programming and methods, an additional pedagogical benefit of conducting online replications is to build competence in these skills.  Even if students’ research does not generally consist of online studies, these replications allow students to build a skillset that can be utilized throughout their academic careers.
...
In this set of replications, we opted for a case-by-case weighing of the costs and benefits of increasing power across all studies. Though we discussed these allocations and bartered allocations as a class, we had a few guiding principles. We first prioritized achieving the participants required for either 80% or original N (whichever was greater) for the maximum number of studies. This benefitted studies which were under the budget allocated for each individual project (i.e. studies that had large effect sizes and short participation times.)  Of those that achieved 80% power with sample sizes that were smaller than the original sample, we reduced the most expensive studies down to N required for 80% power. This allowed us to reach our budget goals with the exception of one particularly expensive study.  Fortunately, this student found external funding to reach 80% power. If this were not possible, we would have likely excluded this study from the pre-registration.

### Workflow selection

Finally, one of the barriers to more reproducible work in psychology is technical: Some of the tools (programmatic statistical analysis, version control) can feel inaccessible or intimidating to beginners. Indeed, our students, who came from a variety of disciplinary backgrounds, had relatively little experience with web-based empirical studies or literate, reproducible data analysis. Yet by the end of the course, all had gained sufficient proficiency with the suite of technical and conceptual tools necessary to carry out a study independently. This experience suggests that it is possible to convey key concepts in sufficient depth that -- within a supportive environment -- students can quickly learn to use a reproducible toolset. 

All replication studies were coded in JavaScript, HTML, and CSS so as to be run in a standard web browser. All analyses were written using R, a free and open-source platform for statistical analysis.  Students wrote their final reports using [R Markdown](http://rmarkdown.rstudio.com/), a "literate programming" environment in which statistical analysis code can be interspersed with text, figures, and tables. This part of our workflow was extremely useful both pedagogically and for encouraging reproducible research practices: each milestone in our course, corresponding to each phase of a research project, required students to fill in additional sections of single unified document. 

Using the template developed by @osc2015 for replication reports (included in our course materials), students first specified the scope and methods of their proposed replication study, including any differences from the original methods. Later, they added the code necessary to analyze their data, and after collecting their full sample, the results of their data analysis (e.g., statistical tests and figures). This method of writing allows students to share a single compiled document via a hyperlink, facilitating review of writing, results, and code together in a single document.^[The current manuscript is written in this fashion as well.] This writing method is also likely to reduce the frequency of statistical reporting errors [@nuijten2015], given that errors are often introduced by transferring results between statistics and word-processing software packages. 

### Ethical approvals

Institutional Review Boards are idiosyncratic across institutions, but we suspect certain ethical concerns will commonly arise when attempting to implement class replication projects.  We address just two, with potential resolutions and comments.

Instructors may first be concerned that IRB approval is not possible within the time constraints of the class.  To mitigate this issue, we suggest contacting the IRB chair well in advance of the class to determine how to either 1) create a standard protocol that encompassses all replications that will be conducted (in which details can be later added and quickly approved), or 2) plan to submit the protocol early in the course, in line with IRB meeting schedules for quick turnarounds. Both issues may be simplified if the study or pool of studies to be replicated is pre-determined and if these studies provide full open-source materials that can be used in the IRB protocol submitted.

In addition, instructors and review boards may believe there is a greater potential for harm given constraints of class. That is, there may be less oversight to each individual project.  However, we note that especially if the original studies have been previously approved by their respective institutional review board, the review process is similar to one might expect out of an advisee/advisor relationship. Particular consideration should be given to concerns of confidentiality of collected data, and we may suggest that for less experienced students, identifiable participant data should only be accessed by the instructor team.  Lastly, this project can also be a useful tool to have class discussions around coersion, harm vs. benefit, and other ethical issues that may arise during reserach.
...
Many statistical factors leading to lowered replicability have been discussed in past work, including analytic flexibility, context dependency, publication bias, and low statistical power among others [e.g., @button2013; @vanbavel2016; @ioannidis2005]. For both reasons of design and scale, our study here cannot disentangle these factors empirically. Nevertheless, the decreases in effect size and evidential value we report seem consistent with two explanations. First, we likely saw a "winner's curse," with initial publications tending to over-estimate effects. Second, we also might have seen a reduction in effect size because the original studies were tailored to their specific context and population, whereas our replications may not have been [@vanbavel2016, but cf. @inbar2016]. Future work would benefit from a statement of constraints on generality so as to provide a guide to the conditions under which an effect is likely to be present [@simons2016]. Both of these explanations are consistent with our observations, and, based on the previous literature [@osf2015,@vanbavel2016], we speculate that both winner's curse and context effects were present to some degree.
...
This kind of cross-class collaboration would require instructors to make class results more broadly discoverable. We can envision a number of possible avenues for this kind of sharing. For example, if some products are shared openly in a repository like the [Open Science Framework](http://osf.io) or [figshare](http://figshare.com) and tagged appropriately, they will be discoverable via search engines. Alternatively, a more structured method for sharing and discovery would be upload to specific replication curation websites like [PsychFileDrawer](http://psychfiledrawer.org) or CurateScience (http://curatescience.org/). Both of these models assume limited coordination across instructors, but more structured collaborations are of course possible. For example, the [Collaborative Replications and Education Project](https://osf.io/wfc6u/), selected a subsample of important and feasible studies for replication and helped provide materials to instructors with the explicit goal of encouraging cross-lab pedagogical replications. 

One challenge for this kind of dissemination is how to curate which projects are released publicly. We are generally optimistic about the contributions of student work, but of course not every student project will be scientifically informative. To take an edge case, imagine a student makes an error in stimulus randomization that is not caught by the instructors until after data collection and that and creates a major confound in the collected data. In this case, we would argue that this work should not be deposited in a targeted replication repository. In the end, these decisions will be the responsibility of the course instructor -- much like decisions about when to publish other research.  
...
For each project, we present a summary of the finding, the key statistical test, the impressionistic outcomes of the replication, and -- where applicable -- our assessement of the source of differences in results (often composed with the advice of the original authors). Figure 3 shows a side-by-side comparison of the key visualization for each study. 
