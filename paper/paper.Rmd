...
Students used JavaScript and HTML/CSS to reimplement their respective target studies in the web browser, following the methods specified by the original authors as closely as possible.
...
## Writing, Preregistration, and Review
...
In addition to as-needed guidance on projects, students advanced through a formal review process in which each student's work was inspected and critiqued several times by both the instructor and the teaching assistants. After selecting their replication targets, students' first milestone was to fill in the methods section of the @osc2015 replication report template, which was reviewed for feasability and concreteness. This section included a power analysis, proposed sample size, and explicit description of differences from the original study (see course materials for complete template).  
...
Student group & \multicolumn{1}{p{2in}}{\raggedright General Undergraduate} & \multicolumn{1}{p{2in}}{\raggedright Advanced Undergraduate or Graduate} & \multicolumn{1}{p{2in}}{\raggedright Sophisticated Graduate} \\
...
Group size & \multicolumn{1}{p{2in}}{\raggedright Full class / medium-size groups (7-8 students)} & \multicolumn{1}{p{2in}}{\raggedright Small groups (2-4) or individuals} & \multicolumn{1}{p{2in}}{\raggedright Individual} \\

Project selection & \multicolumn{1}{p{2in}}{\raggedright Single class replication or small curated list} & \multicolumn{1}{p{2in}}{\raggedright Curated list} & \multicolumn{1}{p{2in}}{\raggedright Open project selection*} \\
...
Workflow tools & \multicolumn{1}{p{2in}}{\raggedright Use GUI tools / instructors implement studies} & \multicolumn{1}{p{2in}}{\raggedright Teach one key open-source tool (e.g., R)} & \multicolumn{1}{p{2in}}{\raggedright Teach full ecosystem of key open-source tools} \\
...
Dissemination & \multicolumn{1}{p{2in}}{\raggedright If meeting pre-specified quality standard} & \multicolumn{1}{p{2in}}{\raggedright With instructor approval} & \multicolumn{1}{p{2in}}{\raggedright As class default (subject to disucssion)} \\
...
Some of these tools can feel inaccessible or intimidating to beginners. Indeed, our students, who came from a variety of disciplinary backgrounds, typically have relatively little experience with web-based empirical studies or literate, reproducible data analysis. Yet by the end of the course, all gain sufficient proficiency with the suite of technical and conceptual tools necessary to carry out a study independently. Our experience suggests that it is possible to convey key concepts in sufficient depth that -- within a supportive environment -- students can quickly learn to use a reproducible toolset. 

Our explicit class goal was to provide students with experiences navigating the full ecosystem of open-source scientific tools. For classes at different levels, however, instructors will likely have different goals. It may be preferrable for an intermediate course to focus on a single programming tool (e.g., R) and to use other GUI-based software for creating experiments, so as to minimize the learning burden for students. And for more introductory research methods course, instructors must gauge whether the added difficulty of a programming compoent is appropriate for the skills and backgrounds of their students.

### Ethical approvals

Institutional Review Boards (IRBs) are idiosyncratic across institutions, but we suspect certain ethical concerns will commonly arise when attempting to implement class replication projects. First, instructors may first be concerned that IRB approval is not possible within the time constraints of the class.  To mitigate this issue, we suggest contacting the IRB chair well in advance of the class to determine whether to create a standard protocol that encompassses all replications that will be conducted, or to determine a timeline for submitting individual protocols very early in the course. Both issues may be simplified if the study or pool of studies to be replicated is pre-selected by instructors. Second, review boards may be concerned about risks particular to the collection of data by students, e.g., lack of debriefing or greater risk of breach of confidentiality due to novice experimenters. We recommend building training around ethical issues into the syllabus of replication-based classes. This practice is both positive for studetns and mitigates potential IRB concerns via explicit training. 
...
One challenge for this kind of dissemination is how to curate which projects are released publicly. We are generally optimistic about the contributions of student work, but of course not every student project will be scientifically informative. To take an edge case, imagine a student makes an error in stimulus presentation that is not caught by the instructors until after data collection. In this case, we would argue that this work should not be deposited in a targeted replication repository. But in the end, these decisions will be the responsibility of the course instructor -- much like decisions about when to publish other research.  
