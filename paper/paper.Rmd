...
We assess the replication p-value to determine whether the key statistic found a significant effect at the traditional p=.05 level. We find this to be true for only `r sum(d$rep_p_value_05)` of the studies. However, the well-documented "dance of the p-values" [@cummings2011] suggests that the inference drawn from a single replication p-value may not be highly informative, and other metrics suchs as the confidence interval and Bayes factor analysis below can give the reader a better understanding of replication success.  

Flag: Are all effects in the hypothesized direction?


```{r}
ggplot(d,aes(x=original_p_value_bins,y=rep_p_value, color=as.factor(d$replicated_TA))) +
  geom_jitter(height=0, width=.4) +
  theme_bw() + 
  geom_hline(yintercept=c(0.05,0.1,0.2), color="gray")
```
...
getBF <- function(t_stat, t_df) {
  return(log10(ttest.tstat(t = t_stat, n1 = t_df, rscale = 1, simple = T)))
};

tstatDF <- d %>%
  filter(original_t_stat != "NA") %>%
  select(project_key, original_t_stat, original_t_df, rep_t_stat, rep_t_df) %>%
  rowwise() %>%
  mutate(origBF = getBF(original_t_stat, original_t_df)) %>%
  rowwise() %>%
  mutate(repBF = getBF(rep_t_stat, rep_t_df))

ggplot(tstatDF, aes(x = origBF, y = repBF, size = rep_t_df/original_t_df)) +
  geom_point()+
  theme_bw()+
  xlim(-2, 30) +
  ylim(-2, 30) +
