...
\%`r round(mean(d$replicated_TA[d$high_fidelity]), digits=2)` 
vs. 
\%`r round(mean(d$replicated_TA[d$high_fidelity == FALSE]), digits=2)`.
There were small numerical effects of being run on Mechanical Turk (
\%`r round(mean(d$replicated_TA[d$original_on_turk == "Yes"]), digits=2)` 
vs. 
\%`r round(mean(d$replicated_TA[d$original_on_turk == "No"]), digits=2)`. 
) and having open materials (
\%`r round(mean(d$replicated_TA[d$open_materials!="No"]), digits=2)` 
vs. 
\%`r round(mean(d$replicated_TA[d$open_materials=="No"]), digits=2)`), though we do not believe these differences are interpretable given our sample size. 
...
Flag: Original effect size not given for auc?

```{r}
d$original_ES <- as.numeric(gsub(".*=(.*)","\\1", d$original_ES_text))
#Ensure that effect size is given in same units... Make beautiful ci chart here...
```
