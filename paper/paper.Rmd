...
Determining appropriate sample sizes is a second major challenge in replication research [@button2013;@simonsohn2015]. Post-hoc power analyses are problematic because of the likelihood of inflation of effect sizes due to the "winner's curse" [@hoenig2001;@button2013], but -- especially with the constraints of a limited budget -- it may be impossible to follow more conservative guidelines in all cases. For example, @simonsohn2015 recommends 2.5x the original sample, which can be feasible for small or under-powered original studies but may lead to impractical or unnecessary recommendations for experiments that were initially large and/or adequately powered. 
...
We next turn to an assessment of the replication $p$-value to determine whether the key statistic found a significant effect at the traditional $p=.05$ level. Only `r sum(d$rep_p_value_05)` of the studies yielded a significant replication $p$-value. The well-documented "dance of the p-values" [@cumming2013] suggests that the inference drawn from a single replication *p*-value may not be  informative, however, and other metrics such as effect sizes and Bayes factors may provide a more nuanced perspective. While we preregistered an analysis determining whether the original reported effect size is included in the 95\% confidence interval found in the replication, we instead opted for a straightforward comparison of point estimates. This choice was motivated by recent discussions of the utility and validity of interpretating confidence intervals in this manner [@morey2016], as well as discrepencies in reporting effect sizes across the articles we included (which made computing confidence intervals difficult in some cases). 

### Effect size and Bayes Factor analysis

We first compared effect sizes from the original and replication studies (Figure 2A). Effect sizes were highly correlated between the two sets of studies 
...
(`r signif(coef(lm(d$rep_es~d$original_ES_d-1))[1], digits = 2)`\% of the original [`r signif(confint(lm(d$rep_es~d$original_ES_d-1))[1], digits = 2)` - `r signif(confint(lm(d$rep_es~d$original_ES_d-1))[2], digits = 2)`]). This estimate should be viewed with caution, however, due to its dependence on one very large effect size estimate ($d > 15$) from a single study.

Next, following @etz2016, we compared Bayes Factors between the original and replication (Figure 2B). The Bayes Factor is an alternative to classical null hypothesis significance testing; it quantifies the evidence in favor of a hypothesis as the ratio of the likelihood of the data under that hypothesis relative to another [@jeffreys1961]. Its proponents point to many advantages over classical $p$-values. For example, it does not privilege the null hypothesis and can provide evidence either *for* or *against* it [see also @scheibehenne2016, @wagenmakers2016]. While the Bayes Factor often agrees with the $p$-value as to which hypothesis is more likely [@wetzels2011], they often substantially disagree on the strength of the evidence. 

```{r, fig.cap="Replication effect size (A) and Bayes factor (B), plotted by the original effect size and Bayes factor, respectively. Point size shows replication N (A) and ratio of test degrees of freedom (B), color indicates subjective replication assessments by the authors. Note that the Sofer et al. (2015) replication showed ES and BF too large to plot easily on these axes and is hence excluded.", fig.pos="ht", fig.height=4}
...
For each project, we present a summary of the finding, the key statistical test, and the outcomes of the replication.  
